package videoGame

var Games = []game{
   {
      name: "donkey kong country",
      link: []link{
         {url: "wikipedia.org/wiki/Donkey_Kong_Country"},
         {
            text: "Jammin' Sam Miller",
            url:  "youtube.com/playlist?list=PLKNuUcvZGX21obTJzXuVHUWRcS2cCBBsP",
         },
      },
      track: []track{
         {
            number: 7,
            name:   "aquatic ambience",
            link: []link{
               {
                  text: "Aquatic Ambience [Restored]",
                  url: "youtube.com/watch?v=-5rAjOjTGtc",
               },
               {
                  text: "Aquatic Ambience [Restored] [2023 Mix]",
                  url: "youtube.com/watch?v=39hGqV42CkM",
               },
            },
         },
      },
   },
   {
      name: "donkey kong country 2",
      link: []link{
         {url: "wikipedia.org/wiki/Donkey_Kong_Country_2"},
         {
            url:  "youtube.com/playlist?list=PLKNuUcvZGX23N53oLt93q4CZRj8m-Il_-",
            text: "Jammin' Sam Miller",
         },
      },
      track: []track{
         {
            number: 15,
            name:   "mining melancholy",
            link: []link{
               {
                  url:  "youtube.com/watch?v=cBK2YIrY62I",
                  text: "Donkey Kong Country 2 - Mining Melancholy [Restored]",
               },
               {
                  url:  "youtube.com/watch?v=qpJbv1JEA4I",
                  text: "Donkey Kong Country 2 - Mining Melancholy [Restored] 2023 Mix",
               },
            },
         },
         {
            number: 19,
            name:   "stickerbush symphony",
            link: []link{
               {
                  url:  "youtube.com/watch?v=qIk6YFTzckc",
                  text: "Donkey Kong Country 2 - Stickerbush Symphony [Restored]",
               },
               {
                  url:  "youtube.com/watch?v=TxP2nqBIC44",
                  text: "Donkey Kong Country 2 - Stickerbush Symphony [Restored] 2024 Mix",
               },
            },
         },
         {
            number: 23,
            name:   "forest interlude",
            link: []link{
               {
                  url:  "youtube.com/watch?v=wxAZfLenfa0",
                  text: "Donkey Kong Country 2 - Forest Interlude [Restored]",
               },
               {
                  url:  "youtube.com/watch?v=AZOZXoDwCzI",
                  text: "Donkey Kong Country 2 - Forest Interlude [Restored] 2025 Mix",
               },
            },
         },
      },
   },
   {
      name: "machinarium",
      link: []link{
         {url: "wikipedia.org/wiki/Machinarium"},
         {
            text: "kaikarden",
            url:  "youtube.com/playlist?list=PLDF2E3F105D56FCE6",
         },
      },
      track: []track{
         {
            number: 0,
            name:   "by the wall",
            link: []link{
               { url:  "youtube.com/watch?v=jex5rtwx94k" },
            },
         },
      },
   },
   {
      name: "mega man x",
      link: []link{
         {url: "wikipedia.org/wiki/Mega_Man_X_(video_game)"},
         {
            text: "Frygoon",
            url:  "youtube.com/playlist?list=PLLVedurB7ZPbdTRgvL53QN_0o8bWO9vL3",
         },
      },
      track: []track{
         {
            number: 12,
            name:   "Chill Penguin (Snow Mountain Stage)",
            link: []link{
               {
                  url:  "youtube.com/watch?v=fcOjHuAD6SI",
                  text: "Mega Man X OST - T12: Chill Penguin (Snow Mountain Stage)",
               },
               {
                  url:  "youtube.com/watch?v=etuuQyKuel4",
                  text: "Mega Man X1 - Chill Peguin Stage(Jazz Remix)",
               },
            },
         },
      },
   },
   {
      name: "pilotwings 64",
      link: []link{
         {url: "wikipedia.org/wiki/Pilotwings_64"},
         {
            url:  "youtube.com/playlist?list=PLWsnao9n727OfUwLfkm1bJjnkwxLNcSNH",
            text: "Pilotwings 64 HD",
         },
         {
            url:  "youtube.com/playlist?list=PLWsnao9n727MUC1DSLXttRIASGxLmTRQb",
            text: "Pilotwings 64 HD Ultra",
         },
         {
            url:  "youtube.com/playlist?list=PLWsnao9n727NrHIiqnZEtD180s9qxL4zl",
            text: "Pilotwings 64 Retro HD+",
         },
      },
      track: []track{
         {
            number: 5,
            name:   "Hang Glider",
            link: []link{
               {
                  url:  "youtube.com/watch?v=h8-OfKFPnjw",
                  text: "2019",
               },
               {
                  url:  "youtube.com/watch?v=aaInC-vCjt0",
                  text: "2020",
               },
               {
                  url:  "youtube.com/watch?v=nv69iNJbqKo",
                  text: "2022",
               },
            },
         },
         {
            number: 9,
            name:   "Rocket Belt",
            link: []link{
               {
                  url:  "youtube.com/watch?v=sAtiUoPB7d4",
                  text: "2019",
               },
               {
                  url:  "youtube.com/watch?v=R9u06H6j8a0",
                  text: "2020",
               },
               {
                  url:  "youtube.com/watch?v=1QI0zjNZJtU",
                  text: "2022",
               },
            },
         },
         {
            number: 20,
            name:   "Sky Diving",
            link: []link{
               {
                  url:  "youtube.com/watch?v=NDlyfygPZDs",
                  text: "2019",
               },
               {
                  url:  "youtube.com/watch?v=cbb0ubUAJqc",
                  text: "2020",
               },
               {
                  url:  "youtube.com/watch?v=uARj6X14qE8",
                  text: "2022",
               },
            },
         },
         {
            number: 26,
            name:   "Birdman",
            link: []link{
               {
                  url:  "youtube.com/watch?v=DROiSgvFsRY",
                  text: "2019",
               },
               {
                  url:  "youtube.com/watch?v=ar8oGTSg5Lk",
                  text: "2020",
               },
               {
                  url:  "youtube.com/watch?v=bsUNUqk-JHg",
                  text: "2022",
               },
            },
         },
         {
            number: 31,
            name:   "Bravissimo!",
            link: []link{
               {
                  url:  "youtube.com/watch?v=mziDsd_SE0M",
                  text: "2019",
               },
               {
                  url:  "youtube.com/watch?v=qzT0yKHN1f8",
                  text: "2020",
               },
               {
                  url:  "youtube.com/watch?v=mdHjSThrRYc",
                  text: "2022",
               },
            },
         },
      },
   },
   {
      name: "portal",
      link: []link{
         {url: "wikipedia.org/wiki/Portal_(video_game)"},
         {
            text: "Rabbit OST",
            url:  "youtube.com/playlist?list=PLH834K1pkcjKfeTgaAhrEPQxaLPBHayJ9",
         },
      },
      track: []track{
         {
            number: 3,
            name:   "self esteem fund",
            link: []link{
               {
                  text: "rabbit ost",
                  url:  "youtube.com/watch?v=9oQYVui9EaU",
               },
               {
                  text: "thyzzie",
                  url:  "youtube.com/watch?v=pT7pVK5q8tk",
               },
            },
         },
      },
   },
   {
      name: "star fox 64",
      link: []link{
         {url: "archive.org/details/starfox-64-soundtrack-cd"},
         {url: "wikipedia.org/wiki/Star_Fox_64"},
         {
            text: "LadyKitsune1116",
            url:  "youtube.com/playlist?list=PLTcAyMjIQ7rJaUcAy1yVaeMQbHnwmzMWy",
         },
      },
      track: []track{
         {
            number: 1,
            name:   "opening theme",
            link: []link{
               {url: "archive.org/details/starfox-64-soundtrack-cd/01+-+Opening+Theme.flac"},
               {url: "youtube.com/watch?v=EC1GyQ2SyUw"},
            },
         },
         {
            number: 2,
            name:   "title theme",
            link: []link{
               {url: "archive.org/details/starfox-64-soundtrack-cd/02+-+Title+Theme.flac"},
               {url: "youtube.com/watch?v=C6EZWbrBVRk"},
            },
         },
         {
            number: 3,
            name:   "select theme",
            link: []link{
               {url: "archive.org/details/starfox-64-soundtrack-cd/03+-+Select+Theme.flac"},
               {url: "youtube.com/watch?v=_l4CmiCPrq0"},
            },
         },
      },
   },
   {
      name: "super mario 64",
      link: []link{
         {url: "google.com/search?q=Koji+Kondo+Super+Mario+64"},
         {url: "wikipedia.org/wiki/Super_Mario_64"},
         {
            url:  "youtube.com/playlist?list=PLB4DC2508C4D0076D",
            text: "RadiatorRampardos",
         },
         {
            url:  "youtube.com/playlist?list=PLu0fouVDrRZM1j1U4lTBgNnLbEX4R4Nld",
            text: "SilvaGunner",
         },
      },
      track: []track{
         {
            number: 9,
            name:   "Dire, Dire Docks",
            link: []link{
               {
                  text: "dire dire docks but it’s boards of canada",
                  url:  "youtube.com/watch?v=7ik1_snr7B8",
               },
               {
                  text: "RadiatorRampardos",
                  url:  "youtube.com/watch?v=Zqa2mgjbOIM",
               },
               {
                  text: "relaxing piano - super mario 64 underwater theme",
                  url:  "youtube.com/watch?v=XRh0Z36RrWY",
               },
               {
                  text: "SilvaGunner",
                  url:  "youtube.com/watch?v=N6G7uFlUyn4",
               },
            },
         },
         {
            number: 35,
            name:   "staff roll",
            link: []link{
               {
                  text: "RadiatorRampardos",
                  url:  "youtube.com/watch?v=o-xRxXYpavU",
               },
               {
                  text: "SilvaGunner",
                  url:  "youtube.com/watch?v=_UmksF3MFXg",
               },
            },
         },
      },
   },
   {
      name: "super mario galaxy 2",
      link: []link{
         {url: "wikipedia.org/wiki/Super_Mario_Galaxy_2"},
         {
            url:  "youtube.com/playlist?list=PLC583980D7806298E",
            text: "RadiatorRampardos",
         },
      },
      track: []track{
         {
            number: 27,
            name:   "Cosmic Cove Galaxy",
            link: []link{
               {url: "youtube.com/watch?v=zcV7RptHqro"},
            },
         },
      },
   },
   {
      name: "super mario world",
      link: []link{
         {url: "wikipedia.org/wiki/Super_Mario_World"},
         {
            url:  "youtube.com/playlist?list=PLu0fouVDrRZNXc8DkkZR2Sx9N5wuotyrO",
            text: "SilvaGunner",
         },
      },
      track: []track{
         {
            number: 3,
            name:   "donut plains",
            link: []link{
               {url: "youtube.com/watch?v=sMqzn_bqK1Q"},
            },
         },
      },
   },
   {
      name: "the elder scrolls iii: morrowind",
      link: []link{
         {url: "wikipedia.org/wiki/The_Elder_Scrolls_III:_Morrowind"},
         {
            text: "ETSLord",
            url: "youtube.com/playlist?list=PL47A1A80CE45D22ED",
         },
      },
      track: []track{
         {
            number: 1,
            name: "Nerevar Rising",
            link: []link{
               {url: "youtube.com/watch?v=OHyY8BFPJZw"},
            },
         },
         {
            number: 2,
            name: "Peaceful Waters",
            link: []link{
               {url: "youtube.com/watch?v=PIlh-wEDYTQ"},
            },
         },
         {
            number: 4,
            name: "over the next hill",
            link: []link{
               {url: "youtube.com/watch?v=llINJR3X31A"},
            },
         },
         {
            number: 6,
            name: "the road most travelled",
            link: []link{
               {url: "youtube.com/watch?v=HR432XFDUQQ"},
            },
         },
         {
            number: 8,
            name: "blessing of vivec",
            link: []link{
               {url: "youtube.com/watch?v=FzZP3IWmTv0"},
            },
         },
         {
            number: 10,
            name: "silt sunrise",
            link: []link{
               {url: "youtube.com/watch?v=83nQz4JCk1s"},
            },
         },
         {
            number: 12,
            name: "shed your travails",
            link: []link{
               {url: "youtube.com/watch?v=_ETl-hlOCBk"},
            },
         },
         {
            number: 14,
            name: "caprice",
            link: []link{
               {url: "youtube.com/watch?v=qCLYm55Vtus"},
            },
         },
      },
   },
   {
      name: "the elder scrolls v: skyrim",
      link: []link{
         {url: "wikipedia.org/wiki/The_Elder_Scrolls_V:_Skyrim"},
         {
            text: " Jeremy Soule • Album",
            url: "youtube.com/playlist?list=OLAK5uy_kOC_oYOG8Rb_bLVukPtxzs2i-fZ65DYg4",
         },
      },
      track: []track{
         {
            number: 5,
            name: "ancient stones",
            link: []link{
               {url: "youtube.com/watch?v=kX-oTQR1ihk"},
            },
         },
      },
   },
   {
      name: "the legend of zelda: majora's mask",
      link: []link{
         {url: "wikipedia.org/wiki/The_Legend_of_Zelda:_Majora's_Mask"},
         {url: "zeldauniverse.net/media/music/majoras-mask-original-soundtrack"},
         {
            text: "Cha Chingo Christiano",
            url:  "youtube.com/playlist?list=PLESFnlO3kNnojaXTldE2ThGR0NpMuA595",
         },
      },
      track: []track{
         {
            number: 19,
            name:   "cremia's carriage",
            link: []link{
               {url: "youtube.com/watch?v=OkWTYpGF_20"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/majorasmaskost/cd2/2-19+Cremias+Carriage.mp3"},
            },
         },
      },
   },
   {
      name: "the legend of zelda: ocarina of time",
      link: []link{
         {url: "wikipedia.org/wiki/The_Legend_of_Zelda:_Ocarina_of_Time"},
         {url: "zeldauniverse.net/media/music/ocarina-of-time-original-soundtrack"},
         {
            text: "Cha Chingo Christiano",
            url:  "youtube.com/playlist?list=PLESFnlO3kNnpqtdBGrIl58Nn9OVzxhgLl",
         },
      },
      track: []track{
         {
            number: 1,
            name:   "title theme",
            link: []link{
               {url: "youtube.com/watch?v=_dPCK3IPa7k"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/01+-+Title+Theme.mp3"},
            },
         },
         {
            number: 6,
            name:   "kokiri forest",
            link: []link{
               {url: "youtube.com/watch?v=rdaJ5pJmNfo"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/06+-+Kokiri+Forest.mp3"},
            },
         },
         {
            number: 12,
            name:   "inside the deku tree",
            link: []link{
               {url: "youtube.com/watch?v=WomOOpj-8o8"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/12+-+Inside+The+Deku+Tree.mp3"},
            },
         },
         {
            number: 19,
            name:   "hyrule field theme",
            link: []link{
               {url: "youtube.com/watch?v=4h6kO9HVMaw"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/19+-+Hyrule+Field+Theme.mp3"},
            },
         },
         {
            number: 28,
            name:   "lon lon ranch",
            link: []link{
               {url: "youtube.com/watch?v=q5Wq8DoYEmU"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/28+-+Lon+Lon+Ranch.mp3"},
            },
         },
         {
            number: 30,
            name:   "kakariko village",
            link: []link{
               {url: "youtube.com/watch?v=rS-Si8vsfbk"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/30+-+Kakariko+Village.mp3"},
            },
         },
         {
            number: 39,
            name:   "zora's domain",
            link: []link{
               {url: "youtube.com/watch?v=JeJC2fxob0g"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/39+-+Zoras+Domp3"},
            },
         },
         {
            number: 40,
            name:   "great fairy fountain",
            link: []link{
               {url: "youtube.com/watch?v=C3xHPcWxzXM"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/40+-+Great+Fairy+Fountain.mp3"},
            },
         },
         {
            number: 41,
            name:   "potion shop",
            link: []link{
               {url: "youtube.com/watch?v=_UN8OcTfOVA"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/41+-+Potion+Shop.mp3"},
            },
         },
         {
            number: 44,
            name:   "temple of time",
            link: []link{
               {url: "youtube.com/watch?v=Sv66WYSQPAc"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/44+-+Temple+Of+Time.mp3"},
            },
         },
         {
            number: 50,
            name:   "sheik's theme",
            link: []link{
               {url: "youtube.com/watch?v=3iV8MkbGm2o"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/50+-+Sheiks+Theme.mp3"},
            },
         },
         {
            number: 57,
            name:   "windmill hut",
            link: []link{
               {url: "youtube.com/watch?v=ms9-vyV0HNo"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/57+-+Windmill+Hut.mp3"},
            },
         },
         {
            number: 68,
            name:   "gerudo valley",
            link: []link{
               {url: "youtube.com/watch?v=gmwdOH6fJ1s"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/ocarinaoftimeost/68+-+Gerudo+Valley.mp3"},
            },
         },
      },
   },
   {
      name: "the legend of zelda: the wind waker",
      link: []link{
         {url: "wikipedia.org/wiki/The_Legend_of_Zelda:_The_Wind_Waker"},
         {url: "zeldauniverse.net/media/music/the-wind-waker-original-soundtrack"},
         {
            text: "Rabbit OST",
            url:  "youtube.com/playlist?list=PLH834K1pkcjJRwrw6j-Ah3FL48SDAsK_z",
         },
      },
      track: []track{
         {
            number: 3,
            name:   "the legendary hero",
            link: []link{
               {url: "youtube.com/watch?v=jNuLYOMwdS0"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/thewindwakerost/cd1/The_Legend_of_Zelda_-_The_Wind_Waker_-_03_-_Legendary_Hero.mp3"},
            },
         },
         {
            number: 38,
            name:   "dragon roost island",
            link: []link{
               {url: "youtube.com/watch?v=02pp7Pb7Bls"},
               {url: "zeldauniverse.s3.amazonaws.com/soundtracks/thewindwakerost/cd1/The_Legend_of_Zelda_-_The_Wind_Waker_-_38_-_Dragon_Island.mp3"},
            },
         },
      },
   },
}

type game struct {
   name  string
   link  []link
   track []track
}

type track struct {
   number int
   name   string
   link   []link
}

type link struct {
   text string
   url  string
}
